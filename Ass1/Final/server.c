...
// In format: http://localhost:1511/mandelbrot/2/zoom/x-coord/y-coord/tile.bmp
// Will be given something like this:
// http://localhost:1511/mandelbrot/2/8/-1.0/0.5/tile.bmp
// uses sscanf to get values for x, y and zoom.
// If values not present in URL, then user will be redirected to a
// fancy mandelbrot viewer that is interative.
void routeRequest(int socket, char requestBuffer[REQUEST_BUFFER_SIZE]) {
    int z = 0;
    double x = 0;
    double y = 0;

    if (sscanf(requestBuffer, "GET /mandelbrot/2/%d/%lf/%lf/tile.bmp HTTP/1.1", &z, &x, &y) < 3) {
        serveHTML(socket);
    } else {
...
// serves fancy mandelbrot viewer to browser
// Viewer is interative and user can use scroll
static void serveHTML (int socket) {
    char* message;
