...
static int foundCard(Game game, Card topCard);
...
playerMove decideMove(Game game) {
    ...
    if (!isValidMove(game, move)) {
        ...
        int index = NOT_FOUND;
        int found = foundCard(game, topCard);
        if(found == FALSE) {
            ...
            if (findMatchingCardSuit(game,cardSuit(topCard)) != NOT_FOUND && found == TRUE) {
                index = findMatchingCardSuit(game,cardSuit(topCard));
                move.action = PLAY_CARD;
                move.card = handCard(game,index);
            } else if (findMatchingCardValue(game,cardValue(topCard)) != NOT_FOUND && found == TRUE) {
                index = findMatchingCardValue(game,cardValue(topCard));
                move.action = PLAY_CARD;
                move.card = handCard(game,index);
            } else if(findMatchingCardColor(game,cardColor(topCard)) != NOT_FOUND && found == TRUE) {
            ...
            if(cardValue(move.card) == DECLARE && found == TRUE){
